...
def run_audit(df, industry, region):
    ...
    for _, row in df.iterrows():
        ...
        # Determine if 'High is Good' (Efficiency/Recycling) 
        # vs 'High is Bad' (Emissions/Waste)
        is_high_good = any(word in metric.lower() for word in ['efficiency', 'recycle', 'share', '%'])
        
        if is_high_good:
            base_score = (current / target) * 100
        else:
            base_score = (target / current) * 100

        # Apply 2026 Strictness Factor
